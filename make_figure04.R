...
selected_states = c('FL', 'KY')

df_error = subset(df_error, state %in% selected_states)

df_error_over_null = subset(df_error_over_null, state %in% selected_states)

# fix the factor order in the data

pal <- c("null-model"="red","linear-scaling"="orange",
         "prospective-scaling"="#00BFC4","retrospective-scaling"="#C77CFF")

lvls  <- names(pal)

df_error$type          <- factor(df_error$type,          levels = lvls)
df_error_over_null$type<- factor(df_error_over_null$type, levels = lvls)
...
for (mystate in selected_states) {
  df1 = subset(df_error, state == mystate)
  df2 = subset(df_error_over_null, state == mystate)
  
  pl[[icount]] <- ggplot(df1, aes(x = weekendingdate, y = err, fill = type)) +
    geom_col(position='dodge') +
    geom_vline(data = subset(df_vline, state == mystate), aes(xintercept = vline_date), 
               linetype = "dashed", color = "black", linewidth = 0.5) +
    scale_x_date(
      date_breaks = "1 month" ,
      date_labels = "%y-%m-%d"
    ) +
    scale_fill_manual(values = pal, breaks = lvls, limits = lvls, drop = FALSE, name = NULL) +   
    guides(fill = guide_legend(nrow = 1)) +
    labs(title = mystate,
         x = "",
         y = "Correction to Reported Value") +
  ...
  pl[[icount]] <- ggplot(df1, aes(x = weekendingdate, y = rel_err, fill = type)) +
    geom_col(position='dodge') +
    geom_vline(data = subset(df_vline, state == mystate), aes(xintercept = vline_date), 
               linetype = "dashed", color = "black", linewidth = 0.5) +
    scale_x_date(
    ...
    labs(title = mystate,
         x = "",
         y = "Relative Correction to Reported Value") +
    theme_minimal(base_size = 12) +
    theme(
      strip.text = element_text(size = 10),
      axis.text.x = element_text(angle = 45, hjust = 1),
  ...
  pl[[icount]] <- ggplot(df2, aes(x = weekendingdate, y = err, fill = type)) +
    geom_col(position='dodge') +
    geom_vline(data = subset(df_vline, state == mystate), aes(xintercept = vline_date), 
               linetype = "dashed", color = "black", linewidth = 0.5) +
    scale_x_date(
    ...
    labs(title = mystate,
         x = "",
         y = "Correction Relative to NULL Correction") +
    theme_minimal(base_size = 12) +
    theme(
      strip.text = element_text(size = 10),
      axis.text.x = element_text(angle = 45, hjust = 1),
...
ggsave(
  filename = "fig/figure_04.pdf",
  plot     = final_plot,          # your patchwork object
  width    = 11, height = 8.5, units = "in",
  device   = cairo_pdf            # nicer fonts in PDFs (optional)
)

# JT uses the 'affinity' program to remove the legend that appears twice
