...
for (ifile in 1:length(process_files)) {
  ...
  new_rows = rbind(data_small, current_small, data_psi)
...
# df_initial -  build it from df_psi
...
df_lm_fit <- df_lm_fit %>% left_join(df_initial[,c('weekendingdate', 'state','perc_hosp_rep')], by = c('weekendingdate', 'state'))
...
# subset to the states we want to plot

selected_states = c('CA', 'IA', 'NV', 'NY', 'TX', 'VT')

df_cor_plot <- subset(df_cor, state %in% selected_states)

df_lm_fit_plot <- subset(df_lm_fit, state %in% selected_states)

p2 <- ggplot(df_lm_fit_plot, aes(x = perc_hosp_rep, y = scaling, color = 'coral')) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, linetype = "dashed", linewidth = 0.5, colour = 'black') +
  geom_text(
    data = df_cor_plot,
  ...
  labs(
    title = "",
    y = "Ratio of final over initial reported value",
    x = "Initial Reporting Fraction"
  ) +
...
ggsave("fig/figure_02.pdf", plot = p2, width = 8.5, height = 11, units = "in")
