...
current_data_path = "data/PROF_state_cov_hosp.rds"
...
current_hosp_data <- current_hosp$conf_cov_admin

current_hosp_dates <- current_hosp$dates

current_perc_hospcov_report <- current_hosp$perc_hospcov_report
...
current_perc_hospcov_report <- current_perc_hospcov_report[ind,]

df_current_hosp = as.data.frame(current_hosp_data)
df_current_hosp$Date = current_hosp_dates

df_current_report = as.data.frame(current_perc_hospcov_report)
df_current_report$Date = current_hosp_dates

tmp1 <- pivot_longer(df_current_hosp, cols = -Date, names_to ='state', values_to = 'cov_adm')
...
col_names <- c('weekendingdate',  'jurisdiction','totalconfc19newadmperchosprep', 'totalconfc19newadm')
new_col_names <- c('weekendingdate', 'state', 'perc_hosp_rep', 'cov_adm', 'report_date', 'issue','scaling')
...
for (ifile in 1:length(process_files)) {
  ...
  max_report_df <- archive_data %>%
    ...
    summarize(max_report = max(totalconfc19newadmperchosprep, na.rm = TRUE)) %>%
  ...
  data_psi$cov_adm = data_small$cov_adm * (1.+ratio)*0.5
  ...
  current_small$scaling = current_small$cov_adm/data_small$cov_adm

  # linear-scaling model if report fraction is x than cov_adm_null = cov_adm * 1/perc_hosp_rep
  
  data_linear <- data_small
  data_linear$issue = 'linear-scaling'
  data_linear$cov_adm = data_small$cov_adm * 1./data_small$perc_hosp_rep
...
df_lm_fit = df_current[,c('weekendingdate', 'state', 'cov_adm', 'scaling')]
...
df_fit <- df_null %>%
  ...
  mutate(
    scaled_cov_adm = (perc_hosp_rep * slope + intercept) * cov_adm
  )

cols_to_remove <- c("cov_adm","intercept","slope")
df_fit <- df_fit %>% select(-all_of(cols_to_remove))
df_fit$issue <- 'retrospective-scaling'
df_fit <- df_fit %>% rename(cov_adm = scaled_cov_adm)
...
df_err_null <- df_current %>%
  select(weekendingdate, state, cov_adm) %>%
  left_join(df_null %>% select(weekendingdate, state, cov_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_null")) %>%
  mutate(err = (cov_adm_current - cov_adm_null),
         type = "null-model") 

df_err_psi_model <- df_current %>%
  select(weekendingdate, state, cov_adm) %>%
  left_join(df_psi %>% select(weekendingdate, state, cov_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_psi_model")) %>%
  mutate(err = (cov_adm_current - cov_adm_psi_model),
         type = "prospective-scaling") 

df_err_retrospective_model <- df_current %>%
  select(weekendingdate, state, cov_adm) %>%
  left_join(df_fit %>% select(weekendingdate, state, cov_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_retrospective_model")) %>%
  mutate(err = (cov_adm_current - cov_adm_retrospective_model),
         type = "retrospective-scaling") 

df_err_linear_model <- df_current %>%
  select(weekendingdate, state, cov_adm) %>%
  left_join(df_linear %>% select(weekendingdate, state, cov_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_linear_model")) %>%
  mutate(err = (cov_adm_current - cov_adm_linear_model),
         type = "linear-scaling")
...
filename = 'figure_05_cov_data.rds'

saveRDS(file = filename, df_cov)
