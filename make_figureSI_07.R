...
p5 <- ggplot(df_error, aes(x = weekendingdate, y = err, fill = type)) +
  ...
  labs(title = "Correction to New Weekly Reported Hospital Admission Value",
       x = "Week Ending Date",
       y = "Correction to New Weekly Reported Hospital Admission Value") +
...
p7 <- ggplot(df_error_over_null, aes(x = weekendingdate, y = err, fill = type)) +
  geom_col(position='dodge') +
  geom_vline(data = df_vline, aes(xintercept = vline_date), 
             linetype = "dashed", color = "black", linewidth = 0.5) +
  geom_vline(data = df_vline, aes(xintercept = vline_date), 
  ...
  labs(title = "",
       x = "Week Ending Date",
       y = "Error Relative to NULL Model Error") +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(size = 10),
    axis.text.x = element_text(angle = 45, hjust = 1),
...
ggsave("fig/figure_SI07.pdf", plot = p7, width = 15, height = 11, units = "in")
