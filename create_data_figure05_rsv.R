...
current_data_path = "data/PROF_state_rsv_hosp.rds"
...
current_hosp_data <- current_hosp$conf_rsv_admin

current_hosp_dates <- current_hosp$dates

current_perc_hosprsv_report <- current_hosp$perc_hosprsv_report
...
current_perc_hosprsv_report <- current_perc_hosprsv_report[ind,]

df_current_hosp = as.data.frame(current_hosp_data)
df_current_hosp$Date = current_hosp_dates

df_current_report = as.data.frame(current_perc_hosprsv_report)
df_current_report$Date = current_hosp_dates

tmp1 <- pivot_longer(df_current_hosp, cols = -Date, names_to ='state', values_to = 'rsv_adm')
...
col_names <- c('weekendingdate',  'jurisdiction','totalconfrsvnewadmperchosprep', 'totalconfrsvnewadm')
new_col_names <- c('weekendingdate', 'state', 'perc_hosp_rep', 'rsv_adm', 'report_date', 'issue','scaling')
...
for (ifile in 1:length(process_files)) {
  ...
  max_report_df <- archive_data %>%
    ...
    summarize(max_report = max(totalconfrsvnewadmperchosprep, na.rm = TRUE)) %>%
  ...
  data_psi$rsv_adm = data_small$rsv_adm * (1.+ratio)*0.5
  ...
  current_small$scaling = current_small$rsv_adm/data_small$rsv_adm

  # linear-scaling model if report fraction is x than rsv_adm_null = rsv_adm * 1/perc_hosp_rep
  
  data_linear <- data_small
  data_linear$issue = 'linear-scaling'
  data_linear$rsv_adm = data_small$rsv_adm * 1./data_small$perc_hosp_rep
...
df_lm_fit = df_current[,c('weekendingdate', 'state', 'rsv_adm', 'scaling')]
...
df_fit <- df_null %>%
  ...
  mutate(
    scaled_rsv_adm = (perc_hosp_rep * slope + intercept) * rsv_adm
  )

cols_to_remove <- c("rsv_adm","intercept","slope")
df_fit <- df_fit %>% select(-all_of(cols_to_remove))
df_fit$issue <- 'retrospective-scaling'
df_fit <- df_fit %>% rename(rsv_adm = scaled_rsv_adm)
...
df_err_null <- df_current %>%
  select(weekendingdate, state, rsv_adm) %>%
  left_join(df_null %>% select(weekendingdate, state, rsv_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_null")) %>%
  mutate(err = (rsv_adm_current - rsv_adm_null),
         type = "null-model") 

df_err_psi_model <- df_current %>%
  select(weekendingdate, state, rsv_adm) %>%
  left_join(df_psi %>% select(weekendingdate, state, rsv_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_psi_model")) %>%
  mutate(err = (rsv_adm_current - rsv_adm_psi_model),
         type = "prospective-scaling") 

df_err_retrospective_model <- df_current %>%
  select(weekendingdate, state, rsv_adm) %>%
  left_join(df_fit %>% select(weekendingdate, state, rsv_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_retrospective_model")) %>%
  mutate(err = (rsv_adm_current - rsv_adm_retrospective_model),
         type = "retrospective-scaling") 

df_err_linear_model <- df_current %>%
  select(weekendingdate, state, rsv_adm) %>%
  left_join(df_linear %>% select(weekendingdate, state, rsv_adm), 
            by = c("weekendingdate", "state"), 
            suffix = c("_current", "_linear_model")) %>%
  mutate(err = (rsv_adm_current - rsv_adm_linear_model),
         type = "linear-scaling") 
...
filename = 'figure_05_rsv_data.rds'

saveRDS(file = filename, df_rsv)
