...
selected_states = c('GA', 'MN', 'MO', 'NH', 'NJ', 'TX')

df_w_fit_plot = subset(df_w_fit, state %in% selected_states)

p3 <- ggplot(df_w_fit_plot, aes(x = weekendingdate, y = flu_adm, color = issue)) +
  # Thicker lines for 'current' and 'null-model'
  geom_line(
    data = subset(df_w_fit_plot, issue %in% c("current", "null-model")),
    linewidth = 1.2
  ) +
  # Thinner lines for all others
  geom_line(
    data = subset(df_w_fit_plot, !issue %in% c("current", "null-model")),
    linewidth = 0.7
  ) +
  ...
  labs(y = "Weekly New Hospital Admission", x = NULL) +
...
plot(p3)
ggsave("fig/figure_03.pdf", plot = p3, width = 8.5, height = 11, units = "in")
# # log scale version
# 
# p3_log <- ggplot(df_w_fit_plot, aes(x = weekendingdate, y = flu_adm, color = issue)) +
#   geom_line() +
#   # scale_y_log10() +
#   scale_y_continuous(trans = "log1p") +  # use log1p 
#   facet_wrap(~ state, ncol = 2, scales = "free_y") +
#   scale_x_date(
#     date_breaks = "1 month" ,
#     date_labels = "%y-%m-%d"
...
# 
# plot(p3_log)
# ggsave("fig/figure_03_log.pdf", plot = p3_log, width = 8.5, height = 11, units = "in")
