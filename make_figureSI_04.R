...
p4 <- ggplot(df_w_fit, aes(x = weekendingdate, y = flu_adm, color = issue)) +
  # Thicker lines for 'current' and 'null-model'
  geom_line(
    data = subset(df_w_fit, issue %in% c("current", "null-model")),
    linewidth = 1.2
  ) +
...
ggsave("fig/figure_SI04.pdf", plot = p4, width = 16, height = 12, units = "in")

# # log scale version
# 
# p4_log <- ggplot(df_w_fit, aes(x = weekendingdate, y = flu_adm, color = issue)) +
#   geom_line() +
#   # scale_y_log10() +
...
# 
# plot(p4_log)
# ggsave("fig/figure_SI04_log.pdf", plot = p4_log, width = 16, height = 12, units = "in")
